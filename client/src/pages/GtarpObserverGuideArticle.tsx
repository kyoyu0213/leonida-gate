...
const BODY = `ストグラやVCR GTA、にじGTA、ホロGTAといった配信者サーバーは、自分が参加しなくても、配信を通じて街の出来事を見守るだけで十分に楽しめる。こうした見守る側の視聴者は、ストグラでは「観測者（かんそくしゃ）」と呼ばれる。この記事では、GTA6 FEEDが、GTARPを観測者として楽しむための見方・心構え・守るべきマナーを、初心者向けに整理した。
...
export default function GtarpObserverGuideArticle() {
  ...
  return (
    <ArticleLayout
      ...
      seoDesc="ストグラやVCR GTA・にじGTA・ホロGTAを配信で楽しむ「観測者」向けガイド。観測者とは何か、観測の始め方（切り抜き・推し・マルチ視点）、守るべきマナー（指示コメント・鳩コメント・お気持ちを避ける「おはし」）まで初心者向けに解説します。"
      title={TITLE}
      titleEn={TITLE_EN}
      icon="👁️"
      date="2026-06-27"
      body={BODY}
      bodyEn={BODY_EN}
      seoTitleEn="A Guide for GTARP Observers | What Is a Kansokusha in Stgra? | GTA6 FEED"
      seoDescEn="A guide for observers (kansokusha) who enjoy Stgra, VCR GTA, Niji GTA, and Holo GTA via streams. What an observer is, how to start observing (clips, a favorite, multi-viewpoint), and the etiquette to follow."
      aiSummary={[
        '配信者サーバー（ストグラ等）は参加しなくても配信を見守るだけで楽しめる。その視聴者をストグラでは「観測者」と呼び、誰の視点で見るかで主役が入れ替わる群像劇として楽しめる。',
        '観測の入口は「切り抜き」「推し一人を追う」「マルチ視点で見比べる」の3つ。大前提として、キャラと中の人を切り離す／ルールは観測者に非公開なので「正しく行われている」前提で見る。',
        'マナーはストグラ公式の「おはし」＝お気持ち・鳩（知り得ない情報を伝える）・指示コメントをしない。疑問符が付きそうなコメントは一度止まる、感情的になったら配信から離れる、も有効。',
      ]}
      aiSummaryEn={[
        'Streamer servers (like Stgra) can be enjoyed just by watching over streams without participating. In Stgra, such viewers are called "observers (kansokusha)," enjoying it as an ensemble drama where the lead swaps depending on whose viewpoint you watch.',
        'There are three entry points to observing: clips, following one favorite, and comparing multiple viewpoints. As premises: separate the character from the person, and since rules are undisclosed to observers, watch on the assumption things are being done correctly.',
        'For etiquette, Stgra\'s official "ohashi" = do not post okimochi (feelings), hato (conveying info a character could not know), or instruction comments. Pausing on comments that would end with "?", and stepping away when emotional, also help.',
      ]}
    >
