...
const BODY = `GTARP（GTAロールプレイ）には、独自の用語や略語が数多く存在する。サーバーのルールページやDiscordの注意書きは、これらの用語を前提に書かれていることが多く、意味を知らないまま参加するとルール違反につながることもある。
...
const TITLE_EN = 'GTARP Glossary | A Beginner-Friendly Guide to RDM, VDM, Meta, and Server Rules';

const BODY_EN = `GTARP (GTA Roleplay) has many of its own terms and abbreviations. Server rule pages and Discord notices are often written on the premise of these terms, and joining without knowing their meanings can lead to rule violations.
...
| Term | Rough Meaning |
| --- | --- |
| RP | Playing while fully acting as a character |
| Meta | Using information your character could not know |
| RDM | Attacking/killing others without reason |
| VDM | Running people over with a vehicle without reason |
| Power Play | One-sided RP that strips away the other's choices |
...
| Roppou | The official rules of that server |
| BAN | Being expelled from the server |
...
export default function GtarpGlossaryArticle() {
  ...
  return (
    <ArticleLayout
      ...
      seoDesc="GTARP（GTAロールプレイ）の必須用語を初心者向けに解説。RDM・VDM・メタ・パワープレイ・NLR・CK・PKといった禁止行為から、瞑想・歪み・心無き・六法・観測者・ストグラなど日本のGTARP文化特有の言い換えまで、意味のまとまりごとに整理した用語辞典です。"
      title={TITLE}
      titleEn={TITLE_EN}
      icon="📖"
      date="2026-06-27"
      body={BODY}
      bodyEn={BODY_EN}
      seoTitleEn="GTARP Glossary | A Beginner-Friendly Guide to RDM, VDM, Meta, and Server Rules | GTA6 FEED"
      seoDescEn="A beginner-friendly guide to essential GTARP (GTA Roleplay) terms. From prohibited acts like RDM, VDM, meta, power play, NLR, CK, and PK, to euphemisms unique to Japan's GTARP culture such as meisou, yugami, kokoronaki, roppou, kansokusha, and Stgra — a glossary organized by groups of meaning."
      aiSummary={[
        'GTARPの必須用語を、基礎概念／禁止・トラブル系／コマンド・システム系／日本のコミュニティ系の4カテゴリに整理した用語辞典。まず覚えたい10語も掲載。',
        'RDM（無差別殺害）・VDM（車での無差別轢殺）・メタ（キャラが知り得ない情報の使用）・パワープレイ・NLR・CK・PKなど、違反になりやすい用語の定義は確度が高い。',
        '瞑想（再起動）・歪み（バグ）・心無き（NPC）・六法（サーバー公式ルール）・観測者・指示コメント・鳩など、日本の配信者サーバー文化特有の言い換えも解説。細部はサーバーごとに異なる。',
      ]}
      aiSummaryEn={[
        'A glossary organizing essential GTARP terms into four categories: basic concepts, prohibited/trouble terms, command/system terms, and Japan-community terms. It also lists the 10 words to learn first.',
        'The definitions of violation-prone terms such as RDM (random killing), VDM (random vehicle killing), meta (using info a character could not know), power play, NLR, CK, and PK are high-certainty.',
        'It also explains euphemisms unique to Japan\'s streamer-server culture, such as meisou (restart), yugami (bug), kokoronaki (NPC), roppou (server official rules), kansokusha (observer), instruction comments, and hato. The details differ by server.',
      ]}
    >
