...
const BODY = `観測者として配信を見るうちに、自分も街の住人として参加してみたくなった人も多いだろう。だが、いざサーバーに参加が認められても、初日に何をすればいいかわからず、戸惑うことは少なくない。この記事では、GTA6 FEEDが、FiveMのGTARPサーバーに初めて参加する人に向けて、街に入る前の準備から初日の立ち回り、避けるべき失敗までを順を追って整理した。
...
操作や進行で迷ったときは、多くのサーバーが用意しているDiscordのOOC・質問用チャンネルや、運営、慣れた住人に尋ねるとよい。ロールプレイ中の状況説明には、行動や状態を文章で示すコマンドが使えるサーバーもあり、通報の専用コマンドが用意されていることもある。こうしたコマンドはサーバーによって異なるため、[FiveMコマンド辞典](/fivem-gtarp/commands)やそのサーバーの案内で確認してほしい。
...
export default function GtarpFirstDayGuideArticle() {
  ...
  return (
    <ArticleLayout
      ...
      seoDesc="FiveMのGTARPサーバーに初めて参加する初心者向けに、初日の立ち回りを解説。最優先で読む六法（ルールブック）、プレイヤー名やボイスチャットなど街に入る前の準備、初起床後にやること、RDM・VDM・メタなど初日に避けるべき失敗まで順を追ってまとめました。"
      title={TITLE}
      titleEn={TITLE_EN}
      icon="🌅"
      date="2026-06-27"
      body={BODY}
      bodyEn={BODY_EN}
      seoTitleEn="A Guide to Your First Day in GTARP | What to Do on Day One of a FiveM Server | GTA6 FEED"
      seoDescEn="A beginner guide to acting on your first day on a FiveM GTARP server: reading the roppou (rulebook) first, pre-entry prep like player name and voice chat, what to do after 'first waking,' and first-day failures to avoid like RDM, VDM, and meta."
      aiSummary={[
        '前提はFiveM導入済み＆サーバー承認済み。街に入る前に何より先に「六法（ルールブック）」を読む。知らずに違反するとBANの危険があるため最優先。',
        '接続前の準備：キャラ設定／プレイヤー名をキャラ名に変更（初期はSteam名）／ボイスチャットをオンにしてマイク確認／半角英数に切替。右上の「outdated」表示は運営向けで無視してよい。',
        '初日は焦らず街を歩いて雰囲気を掴み、生活職（飲食・タクシー・メカニック等）から始めるのが無難。RDM・VDM・メタ・パワープレイ・OOC持ち込みは避ける。困ったらDiscordのOOC・質問チャンネルで。',
      ]}
      aiSummaryEn={[
        'Premise: FiveM installed and server approved. Before entering the city, read the "roppou (rulebook)" first of all — top priority, since violating it unknowingly risks a ban.',
        'Pre-connection prep: set up your character, change the player name to your character name (it defaults to your Steam name), turn on voice chat and test the mic, and switch to half-width input. The "outdated" notice at the top right is for operators and can be ignored.',
        'On day one, don\'t rush — walk around to grasp the mood and start with a livelihood job (restaurant, taxi, mechanic). Avoid RDM, VDM, meta, power play, and bringing in OOC. When stuck, ask in Discord OOC/question channels.',
      ]}
    >
