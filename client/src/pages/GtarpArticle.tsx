...
次に重要なのがサーバー選びだ。サーバーによってはホワイトリスト（参加審査）が必要で、雰囲気もRPの厳格さも大きく異なる。いきなり大手や配信者サーバーを狙うより、まずは気になるサーバーの配信を見て空気感をつかみ、初心者を歓迎している一般向けサーバーから始めると失敗が少ない。日本のサーバーを探すなら、本サイトの[FiveMサーバー 住民募集板](/servers)も活用してほしい。
...
export default function GtarpArticle() {
  ...
  return (
    <ArticleLayout
      ...
      body={BODY}
      bodyEn={BODY_EN}
      seoTitleEn="What Is GTARP? | A Guide to Living a Second Life in GTA5 | GTA6 FEED"
      seoDescEn="What GTARP (GTA Roleplay) is. Roles and jobs, rules such as IC/OOC, RDM, and metagaming, streaming culture, and where it is headed in the GTA6 era, explained for beginners."
      aiSummary={[
      ...
      aiSummaryEn={[
        'GTARP is roleplay where you play roles such as police, civilians, and criminals on servers like FiveM, living as if it were a life.',
        'It has rules such as the IC/OOC distinction and bans on RDM and metagaming, and developed alongside streaming culture (NoPixel and Sutogura).',
        'Getting started assumes PC GTA5 plus FiveM. How it will be playable in the GTA6 era is undetermined, with the focus on the migration of the foundation.',
      ]}
    >
