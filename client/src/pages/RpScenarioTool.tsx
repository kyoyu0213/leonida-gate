...
const SLOTS: Record<'PLACE' | 'PERSON' | 'OBJECT' | 'MOTIVE', string[]> = {
  ...
  PERSON: ["見知らぬ配達人","借金取り","かつての相棒","覆面の依頼人","酔った常連客","新人の警官","地元の顔役","嗅ぎ回る記者","匿名の内部告発者","羽振りのいい商人","訳ありの流れ者","古い馴染み","声だけの相手","若いチンピラ","引退したはずの元プロ"],
  OBJECT: ["中身の分からないバッグ","なくしたはずの鍵","札束","盗難車のキー","一本のUSBメモリ","高価な腕時計","封を切っていない手紙","血のついた上着","誰かの携帯電話","中身不明の小包","偽造書類の束","古い写真","名前だけ書かれたメモ"],
...
// テンプレートは指示の文面が基本。語を埋めると不自然になる2本だけ言い回しを直している
// （business の1本目は「橋の下の経営は順調」「札束を持ちかけてきた」、drama の2本目は
//   「かつてのかつての相棒」「借金を返す気持ちが揺れる」が出るため）。
const GENRES: Record<ScenarioGenre, GenreDef> = {
  crime: {
    role: '犯罪者/ギャング', mood: 'シリアス',
    templates: [
      "{PLACE}で{PERSON}から{OBJECT}を受け取れ——それが今回の仕事だ。",
      "{OBJECT}を{PLACE}まで運ぶ。{MOTIVE}ための一世一代の取引。",
      "{PERSON}の依頼は単純だった。{PLACE}で{OBJECT}を手に入れること。",
    ],
  },
  emergency: {
    role: '警察/EMS', mood: 'シリアス',
    templates: [
  ...
  business: {
    role: '経営者/民間', mood: 'ドライ',
    templates: [
      "商売は順調——のはずが、{PLACE}に現れた{PERSON}が{OBJECT}がらみの話を持ちかけてきた。",
...
/** テンプレートのスロットを埋める。同じスロットが2回出ても同じ語で埋める。 */
function fill(template: string): string {
  const picked: Partial<Record<keyof typeof SLOTS, string>> = {};
  return template.replace(/\{(PLACE|PERSON|OBJECT|MOTIVE)\}/g, (_, k: keyof typeof SLOTS) => (picked[k] ??= R(SLOTS[k])));
}
