...
const BODY = `「FiveM」と「GTARP」は、しばしば同じものとして語られる。だが、この二つはそもそもレイヤー（層）が違う。ひとことで言えば、FiveMは“遊ぶための土台”であり、GTARPは“その土台の上で行われる遊び方”である。GTA6 FEEDが、混同されがちなこの二つの関係を整理する。
...
| Aspect | FiveM (foundation) | GTARP (way to play) |
| --- | --- | --- |
| Positioning | Foundation / platform | Way to play / genre |
| Content | Mechanism to connect to custom servers | Life roleplay where you play a role |
| Range of play | Anything, such as RP, racing, survival | Specialized in roleplay |
| Premise | PC version of GTA5 + dedicated client | Mostly takes place on FiveM |
...
export default function FivemVsGtarpArticle() {
  return (
    <ArticleLayout
      ...
      seoDesc="FiveMとGTARPは何が違うのか。FiveMは遊ぶための土台、GTARPはその上で行われる遊び方。混同されがちな両者の関係を、比較表とともに初心者向けに整理します。"
      title={TITLE}
      titleEn={TITLE_EN}
      icon="⚖️"
      body={BODY}
      bodyEn={BODY_EN}
      seoTitleEn="What Is the Difference Between FiveM and GTARP? | The Foundation and the Way to Play | GTA6 FEED"
      seoDescEn="What is the difference between FiveM and GTARP? FiveM is the foundation for playing, and GTARP is the way to play on top of it. We sort out the easily confused relationship between the two, with a comparison table, for beginners."
      aiSummary={[
      ...
      aiSummaryEn={[
        'FiveM is the "foundation (platform) for playing," while GTARP is the "way to play (genre) on top of it"; they are on different layers.',
        'There are ways to play other than RP (such as racing) on FiveM, and GTARP is one representative genre among them; they are closely tied but can be sorted out as separate things.',
        'In 2026, the competitors alt:V and RAGE:MP ended and the foundation was consolidated into FiveM. What happens to the foundation in the GTA6 era is the next focus.',
      ]}
    >
