...
2025年9月23日、NoPixelは次世代版「NoPixel V」を発表し、これがRockstar Gamesとの直接協業によって開発されていることを明らかにした。RP専用サーバーがRockstar Games Launcherなどを通じて直接提供される、この種としては初の公式提携である。Rockstar側も公式アカウントでNoPixelチームへの支援を表明した。
...
### MODの公式販売所「Cfx Marketplace」が稼働を始めた

2026年1月12日、RockstarはFiveM・RedM向けの公式MODストア「[Cfx Marketplace](https://marketplace.cfx.re/)」を立ち上げた。これは選定されたクリエイターがMODを販売・配布できる公式の電子ストアで、無料のものもあるが多くは有料だ。テーマパークのMODが129.99ドル、クリエイターパックは最大389.99ドルといった高額な商品も並ぶ。かつてFiveMを「海賊行為を助長する非公認サーバー」と非難していたRockstarが、RP向けMODの売買を公式の枠組みに取り込んだことになる。長らく非公式・グレーだったRPの創作経済が、Rockstar公認のもとで制度化されつつある動きであり、後述する「過度な収益化」への批判とも表裏一体の出来事だ。
...
- NoPixelは2025年9月23日、Rockstarとの公式提携による「NoPixel V」を発表した。
