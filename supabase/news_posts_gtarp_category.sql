-- ============================================================================
--  ニュースカテゴリに「GTARP(gtarp)」を追加するマイグレーション
--  Supabase → SQL Editor に貼って Run。
--  前提：news_posts.sql / news_posts_topic_category.sql 適用済み。
--
--  既存DBでは news_posts の create table が再実行されないため、
--  CHECK制約の貼り替えと、作成／更新RPCのカテゴリ判定の更新をここで行う。
--  （news_posts_topic_category.sql で 'topic' を足したときと同じ手順）
--
--  ※ 権限について
...
--     CATEGORIES、client/src/pages/AdminNews.tsx の CATS）にも 'gtarp' を
--     追加済み。DBとコードのどちらか片方だけだと、管理画面から GTARP を
--     選んだ投稿が 'invalid category' で弾かれるので必ず両方を適用すること。
-- ============================================================================

-- 1) category の CHECK 制約を 'gtarp' を含む集合に貼り替える
alter table public.news_posts
  drop constraint if exists news_posts_category_check;
alter table public.news_posts
  add constraint news_posts_category_check
  check (category in ('release', 'topic', 'update', 'speculation', 'event', 'gtarp'));

-- 2) 作成RPC：カテゴリ判定に 'gtarp' を許可
...
begin
  ...
  if p_category not in ('release', 'topic', 'update', 'speculation', 'event', 'gtarp') then
