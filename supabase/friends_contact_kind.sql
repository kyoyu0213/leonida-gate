-- ============================================================================
--  フレンド募集：連絡先の種類（contact_kind）を追加
--  Supabase → SQL Editor に貼って Run（1回でOK）。
--  friends.sql / friends_crews_author_editdelete.sql / friends_gender.sql 適用済み前提。
--
--  内容:
--   1. friends に contact_kind 列を追加（'discord'|'psn'|'xbox'|'rockstar'|'link'|null）。
--      投稿者が連絡先の種類を明示できる。未指定(null)は従来どおりプラットフォームから自動判定。
--   2. create_friend / update_own_friend を再作成（末尾に p_contact_kind を default null で追加）。
--      既存の名前つき呼び出しも default で通るので旧フロントも壊れない。
--
--  追加型の変更なので、既存の募集・返信・投票は影響を受けません（トランザクション実行）。
-- ============================================================================
begin;

-- 1) 列追加（公開列）
alter table public.friends add column if not exists contact_kind text;
grant select (contact_kind) on public.friends to anon, authenticated;

-- 2) create_friend 再作成（末尾に p_contact_kind を追加）
drop function if exists public.create_friend(text, text, text, text, text, text, text, text, text, text, text, text, text);
...
declare
  ...
  v_ck text := nullif(btrim(p_contact_kind), '');
  v_haystack text;
begin
  ...
  if v_ck is not null and v_ck not in ('discord', 'psn', 'xbox', 'rockstar', 'link') then
    raise exception 'invalid contact_kind';
  end if;
  ...
  insert into friends
    (title, platform, play_style, voice_chat, active_time, age_range, body, contact,
     author_name, gender, contact_kind, delete_key_hash, thread_id, status, ip, ua, anon_id, ip_hash, ip_subnet)
  values (
    trim(p_title),
...
grant execute on function
  public.create_friend(text, text, text, text, text, text, text, text, text, text, text, text, text, text) to anon;

-- 3) update_own_friend 再作成（末尾に p_contact_kind を追加）
-- これまで CREATE OR REPLACE で引数を増やしてきた結果、12引数(旧)・13引数(gender)の
-- オーバーロードが残っている可能性があるので、明示的に削除してから作り直す（PostgREST の
-- 関数選択が曖昧にならないように）。
drop function if exists public.update_own_friend(uuid, text, text, text, text, text, text, text, text, text, text, text);
drop function if exists public.update_own_friend(uuid, text, text, text, text, text, text, text, text, text, text, text, text);
...
begin
  ...
  if v_ck is not null and v_ck not in ('discord', 'psn', 'xbox', 'rockstar', 'link') then raise exception 'invalid contact_kind'; end if;
...
grant execute on function
  public.update_own_friend(uuid, text, text, text, text, text, text, text, text, text, text, text, text, text) to anon;
